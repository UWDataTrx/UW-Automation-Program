...
def create_test_merged_file():
    ...
    data = {
        ...
        'NDC': [f'{np.random.randint(10000, 99999)}-{np.random.randint(100, 999)}-{np.random.randint(10, 99)}' for _ in range(num_rows)],
        'MemberID': [f'MBR{np.random.randint(100000, 999999)}' for _ in range(num_rows)],
        'DATEFILLED': [
        ...
        'PHARMACYNPI': [f'{np.random.randint(1000000, 9999999)}' for _ in range(num_rows)],
    ...
    print(f"Created test merged_file.xlsx with {num_rows} rows and {len(df.columns)} columns")
