...
try:
    ...
    REQUIRED_COLUMNS = getattr(AppProcessingConfig, "REQUIRED_COLUMNS", ["DATEFILLED", "SOURCERECORDID", "NDC", "MemberID"])
except ImportError:
...
class RowIDErrorAnalyzer:
    ...
    def analyze_merged_file_error(self, file_path="merged_file.xlsx"):
        ...
        if not os.path.exists(file_path):
            ...
            return self.create_error_report("FILE_NOT_FOUND", f"File {file_path} does not exist")
        ...
        try:
            ...
            report = self.generate_analysis_report(analysis_results, rowid_test_results, file_path)
    ...
    def analyze_dataframe_structure(self, df):
        ...
        results['missing_required_columns'] = [col for col in required_cols if col not in df.columns]
    ...
    def test_rowid_creation(self, df):
        ...
        try:
            ...
            if available_sort_cols:
                ...
                self.logger.info(f"✓ Sort and RowID creation successful (sorted by: {available_sort_cols})")
            else:
                results['sort_and_create'] = {
                    'success': False,
    ...
    def generate_analysis_report(self, structure_analysis, rowid_tests, file_path):
        ...
        report = {
            ...
            'recommendations': self.generate_recommendations(structure_analysis, rowid_tests),
            'fix_suggestions': self.generate_fix_suggestions(structure_analysis, rowid_tests)
        }
