...
def process_data():
    ...
    try:
        ...
        if not file_paths.get('reprice'):
            write_shared_log("tier_disruption.py", "No reprice/template file provided.", status="ERROR")
        ...
        write_shared_log("tier_disruption.py", f"Processing failed: {e}", status="ERROR")
        raise
