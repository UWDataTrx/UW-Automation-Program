...
try:
    ...
    if "affiliate_ingred_cost" in data.columns and "affiliate_disp_fee" in data.columns:
        data["gross_cost"] = data["affiliate_ingred_cost"].fillna(0) + data["affiliate_disp_fee"].fillna(0)
    else:
        print("[WARNING] affiliate_ingred_cost or affiliate_disp_fee column missing; gross_cost will not be available.")
...
if claims_df is not None and not data.empty:
    print("[INFO] Processing claims file for pricing update (Drug Name or NDC match) using fast vectorized merge...")
    # Normalize drug names and NDCs in both DataFrames
    claims_df['drug_name_norm'] = claims_df['Drug Name'].astype(str).str.lower().str.strip()
    import re
    def clean_ndc(val):
        if pd.isnull(val):
            return ''
    ...
    print('[DEBUG] Sample normalized drug_name_norm in claims:', claims_df['drug_name_norm'].drop_duplicates().head(10).tolist())
    if 'ndc_norm' in claims_df.columns:
        print('[DEBUG] Sample normalized ndc_norm in claims:', claims_df['ndc_norm'].drop_duplicates().head(10).tolist())
    print('[DEBUG] Sample normalized drug_name_norm in data:', data['drug_name_norm'].drop_duplicates().head(10).tolist())
    if 'ndc_norm' in data.columns:
        print('[DEBUG] Sample normalized ndc_norm in data:', data['ndc_norm'].drop_duplicates().head(10).tolist())
    ...
    if 'ndc_norm' in claims_df.columns and 'ndc_norm' in data.columns:
        ...
        print(f'[DEBUG] Number of rows with matched gross_cost by NDC: {(~merged["gross_cost"].isna()).sum()} out of {len(merged)}')
    # Step 2: For rows with no match, try to match on drug name
    mask_no_match = merged['gross_cost'].isna()
    drug_map = data.drop_duplicates('drug_name_norm').set_index('drug_name_norm')['gross_cost']
    merged.loc[mask_no_match, 'gross_cost'] = merged.loc[mask_no_match, 'drug_name_norm'].map(drug_map)
    print(f'[DEBUG] Number of rows with matched gross_cost after drug name fallback: {(~merged["gross_cost"].isna()).sum()} out of {len(merged)}')
    # Fill Total Cost
    total_cost_col = None
    ...
    if total_cost_col is None:
        print("[ERROR] Could not find a 'Total Cost' column in Claims For Analysis.csv.")
    else:
        merged[total_cost_col] = merged['gross_cost'].where(merged['gross_cost'].notnull(), 'N/A')
        output_path = claims_path.replace('.csv', '_wf.csv')
        merged.drop(['drug_name_norm', 'ndc_norm', 'gross_cost'], axis=1, inplace=True, errors='ignore')
        merged.to_csv(output_path, index=False)
        print(f"[SUCCESS] Output written to '{output_path}' with {len(merged)} rows. 'Total Cost' column updated.")