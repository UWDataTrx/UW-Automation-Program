...
def read_drug_data(file_path):
    ...
    try:
        ...
        required_cols = ['drug_name', 'strength', 'quantity', 'affiliate_ingred_cost', 'affiliate_disp_fee', 'days_supply']
        missing_cols = [col for col in required_cols if col not in data.columns]
        if missing_cols:
            messagebox.showerror("Missing Columns", f"The following required columns are missing: {', '.join(missing_cols)}")
            return None
...
def display_drug_details(drug_names, data, show_strength=True, show_quantity=True, dosing=None, quantity=None,
    ...
    for drug in drug_names:
        ...
        drug_info = data[data['drug_name'].str.lower().str.contains(drug.strip().lower(), na=False)]
        if not drug_info.empty:
            if show_strength:
                drug_info.loc[:, 'strength'] = drug_info['strength'].fillna('').astype(str)
            else:
                drug_info["strength"] = ""
            ...
            if dosing:
                drug_info = drug_info[drug_info['strength'].str.lower().str.contains(dosing.strip().lower(), na=False)]
            if quantity:
                drug_info = drug_info[drug_info['quantity'].str.lower().str.contains(quantity.strip().lower(), na=False)]
            if supply_duration:
                drug_info = drug_info[drug_info['30/90'].isin(supply_duration)]

            if not drug_info.empty:
                drug_pt = drug_info.groupby(["drug_name", "quantity", "30/90", "strength"]).agg(
                    {'gross_cost': ['mean', mode_func]}).reset_index()
                drug_pt.columns = ['drug_name', '30/90', 'strength', 'quantity', 'mean_cost', 'mode_cost']
                drug_pt['mean_cost'] = drug_pt['mean_cost'].apply(lambda x: f"${x:,.2f}")
                drug_pt['mode_cost'] = drug_pt['mode_cost'].apply(lambda x: f"${x:,.2f}" if x is not None else "N/A")
                results.append(drug_pt)
            else:
                # No results after filtering
                results.append(pd.DataFrame(columns=["drug_name", "quantity", "30/90", "strength", "mean_cost", "mode_cost"]))
        else:
            results.append(pd.DataFrame(columns=["drug_name", "quantity", "30/90", "strength", "mean_cost", "mode_cost"]))
    if results:
...
button_rebate = ctk.CTkButton(root, text="Savings Analysis Lookup", command=rebate_clicked, font=("Oswald", 16, 'bold'))
button_rebate.pack(side=ctk.BOTTOM, pady=10)


button_export_csv = ctk.CTkButton(root, text="Export Selected", command=export_to_parquet, font=("Oswald", 16, 'bold'))
button_export_csv.pack(side=ctk.BOTTOM, pady=10)


style = ttk.Style()
style.theme_use("clam")
style.configure("Treeview.Heading", font=("Oswald", 14, 'bold'), foreground='#333F48', background='dark grey',
                relief='flat')
style.configure("Treeview", font=("Oswald", 14), rowheight=25, background='#333F48', fieldbackground='#333F48',
                bordercolor="#333F48")


columns = ["Drug Name", "Quantity", "30/90 Day Supply", "Dosing", "Average Cost", "Mode Cost"]
