...
def display_drug_details(
    ...
    search_terms = "|".join(
        [re.escape(name.strip().lower()) for name in drug_names if name.strip()]
    )
    filtered = data[
        data["drug_name_lower"].str.contains(search_terms, na=False, regex=True)
    ]
...
for col in columns:
    display_name = (
        "Average Cost" if col == "mean_cost" else col.replace("_", " ").title()
    )
...
button_export = ctk.CTkButton(
    frame1,
    text="Export to Parquet",
    command=export_to_parquet,
    font=("oswald", 16, "bold"),
)
button_export.grid(row=6, column=3, padx=10, pady=10)

button_rebate = ctk.CTkButton(
    frame1, text="Rebate Lookup", command=rebate_clicked, font=("oswald", 16, "bold")
)
