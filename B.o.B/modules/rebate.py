...
def vlookup_rebate(parquet_file, rebate_file):
    ...
    if not os.path.exists(parquet_file):
        messagebox.showerror(
            "File Not Found", f"The file '{parquet_file}' does not exist."
        )
        return None
    if not os.path.exists(rebate_file):
        messagebox.showerror(
            "File Not Found", f"The file '{rebate_file}' does not exist."
        )
        return None
    try:
    ...
    missing_rebate_cols = [
        col for col in required_rebate_cols if col not in rebate_df.columns
    ]
    if missing_df_cols:
        messagebox.showerror(
            "Missing Columns",
            f"The following columns are missing in {parquet_file}: {', '.join(missing_df_cols)}",
        )
        return None
    if missing_rebate_cols:
        messagebox.showerror(
            "Missing Columns",
            f"The following columns are missing in {rebate_file}: {', '.join(missing_rebate_cols)}",
        )
        return None
    ...
    merged_df = pd.merge(
        df, rebate_df, left_on="Drug Name", right_on="drug_name", how="left"
    )
...
def display_data(merged_df):
    ...
    label = ctk.CTkLabel(
        frame, text="Savings Analysis Lookup", font=("Oswald", 24, "bold")
    )
    ...
    all_button = ctk.CTkButton(
        root, text="Select All Rows", command=select_all, font=("Oswald", 16, "bold")
    )
