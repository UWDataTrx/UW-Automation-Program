...
def merge_files():
    ...
    if not os.path.exists(file1_path):
        messagebox.showerror(
            "File Not Found", f"The file '{file1_path}' does not exist."
        )
        return
    if not os.path.exists(file2_path):
        messagebox.showerror(
            "File Not Found", f"The file '{file2_path}' does not exist."
        )
        return
    ...
    if "SOURCERECORDID" not in df1.columns:
        messagebox.showerror(
            "Missing Column", "'SOURCERECORDID' column is missing in the first file."
        )
        return
    ...
    if "SOURCERECORDID" not in df2.columns:
        messagebox.showerror(
            "Missing Column", "'SOURCERECORDID' column is missing in the second file."
        )
        return
    ...
    try:
        ...
        unmatched_count = df1[~df1["SOURCERECORDID"].isin(df2["SOURCERECORDID"])].shape[
            0
        ]
        ...
        unmatched_label.configure(
            text=f"Unknown Claims Removed From Tool: {unmatched_count}"
        )
    except Exception as e:
...
file1_label = ctk.CTkLabel(
    root, text="No file selected", font=("Aptos Narrow", 16, "bold")
)
file1_label.grid(row=0, column=0, pady=2, padx=30)
file1_button = ctk.CTkButton(
    root,
    text="Select Uploaded File",
    command=open_file_1,
    width=200,
    height=30,
    fg_color="#0784b5",
    font=("Aptos Narrow", 16),
    corner_radius=2,
)
...
file2_button = ctk.CTkButton(
    root,
    text="Select File From Tool",
    command=open_file_2,
    width=200,
    height=30,
    fg_color="#0784b5",
