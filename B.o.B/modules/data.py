...
def process_data():
    ...
    e_data = pd.read_excel(e_file_path, sheet_name="Alternatives NDC")[
        ["NDC", "Tier", "Alternative"]
    ]

    merged_data = pd.merge(
        claims_data, medi_data, left_on="NDC", right_on="NDC", how="left"
    )
    merged1_data = pd.merge(
        u_data, merged_data, left_on="NDC", right_on="NDC", how="left"
    )
    merged2_data = pd.merge(
        e_data, merged1_data, left_on="NDC", right_on="NDC", how="left"
    )

    column_mapping = {"Tier_x": "Exclusive Tier", "Tier_y": "Universal Tier"}
    ...
    recent_data = merged2_data[
        (merged2_data["DATEFILLED"] >= starting_point)
        & (merged2_data["DATEFILLED"] <= latest_date)
    ]
    ...
    filtered_data = recent_data[
        (recent_data["Logic"] >= 5)
        & (recent_data["Logic"] <= 10)
        & (recent_data["Maint Drug?"] == "Y")
    ]

    filtered1_data = filtered_data[
        ~filtered_data["Product Name"].str.contains(r"\balbuterol\b", case=False)
    ]

    data = filtered1_data[
        ~filtered1_data["Alternative"].str.contains(
            "Covered|Use different NDC", case=False, regex=True
        )
    ]
