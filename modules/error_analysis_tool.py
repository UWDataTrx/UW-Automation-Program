...
"""
        ...
        if len(error_df[error_df['Status'] == 'FILE_ERROR']) > 0:
            report += "- Multiple file errors detected. Check file permissions and paths.\n"
        ...
        if len(error_df[error_df['Status'] == 'SYSTEM_ERROR']) > 0:
            report += "- System errors detected. Review system resources and dependencies.\n"
...
def get_error_summary(days_back=7):
    ...
    try:
        ...
        if error_df.empty:
            return {"total_errors": 0, "message": "No errors found in the specified period"}
        
        summary = {
            "total_errors": len(error_df),
            "error_types": error_df['Status'].value_counts().to_dict(),
            "users_with_errors": error_df['User'].nunique(),
            "most_affected_user": error_df['User'].value_counts().index[0] if len(error_df) > 0 else None,
            "most_common_script": error_df['Script'].value_counts().index[0] if len(error_df) > 0 else None,
            "date_range": f"Last {days_back} days",
            "latest_error": error_df.iloc[0]['Timestamp'].strftime('%Y-%m-%d %H:%M') if len(error_df) > 0 else None
        }
...
if __name__ == "__main__":
    ...
    if not recent_errors.empty:
        for _, error in recent_errors.head(3).iterrows():
            print(f"   {error['Timestamp'].strftime('%Y-%m-%d %H:%M')} - {error['User']} - {error['Status']}")
