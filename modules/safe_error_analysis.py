...
def get_user_errors_safe(username, days_back=7):
    ...
    user_errors = [
        error for error in all_errors if username.lower() in error["user"].lower()
    ]
    return user_errors
...
def generate_error_summary():
    ...
    if recent_errors:
        ...
        sorted_scripts = sorted(
            scripts_with_errors.items(), key=lambda x: x[1], reverse=True
        )
    ...
    if recent_success:
        ...
        print(
            f"Latest: {latest_success['timestamp']} - {latest_success['user']} - {latest_success['status']}"
        )
