...
from modules.audit_helper import (log_file_access,
...
output_path = Path("_Rx Claims for LBL.xlsx").resolve()
input_files = []
try:
...
def main():
    ...
    # Start audit session
    log_user_session_start("rx_lbl.py")
    write_audit_log("rx_lbl.py", "Processing started.")
    ...
    try:
        ...
        if "lbl" not in paths:
            ...
            paths["lbl"] = filedialog.askopenfilename(
                title="Select LBL Template File"
            )

        template_path = Path(paths["lbl"]).resolve()
        output_path = Path("_Rx Claims for LBL.xlsx").resolve()
        ...
        # Log file access
        log_file_access("rx_lbl.py", paths["reprice"], "LOADING")
        log_file_access("rx_lbl.py", paths["lbl"], "LOADING")
        ...
        try:
            ...
            make_audit_entry("lbl.py", f"Sheet loading failed: {e}", "DATA_ERROR")
        ...
        logger.info(f"LBL output saved to: {output_path}")
        logger.info("LBL file created successfully.")

        # Log successful completion
        make_audit_entry(
            "lbl.py", f"Successfully generated LBL file: {output_path}", "INFO"
        )
        log_file_access("lbl.py", str(output_path), "CREATED")

        write_audit_log("lbl.py", "LBL file created successfully.")
        show_message(awp, ing, total, rxs)
        messagebox.showinfo("Processing Complete", "Processing complete")

    except Exception as e:
        logger.exception("An error occurred during LBL processing")
        make_audit_entry(
            "lbl.py", f"Processing failed with error: {str(e)}", "SYSTEM_ERROR"
        )
        write_audit_log("lbl.py", f"An error occurred: {e}", status="ERROR")
        messagebox.showerror("Error", f"An error occurred: {e}")
    finally:
        # End audit session
        log_user_session_end("lbl.py")
        root.quit()
