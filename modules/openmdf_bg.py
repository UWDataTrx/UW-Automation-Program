...
def load_data_files(file_paths):
    ...
    mdf = pd.read_excel(
        file_paths["mdf_disrupt"], sheet_name="Open MDF NDC", usecols=["NDC", "Tier"], engine="openpyxl"
    )
    ...
    for col in ["pharmacy_npi", "pharmacy_nabp"]:
        if col in network.columns:
            ...
            network[col] = network[col].apply(
                lambda val: (
                    str(int(float(val)))
                    if val.replace(".", "").replace("e", "").replace("-", "").replace("+", "").isdigit()
                    else str(val)
                )
            )
    ...
    logger.info(
        f"network['pharmacy_is_excluded'] value counts before merge: {network['pharmacy_is_excluded'].value_counts(dropna=False).to_dict()}"
    )
...
def merge_data_files(claims, reference_data, network):
    ...
    # Standardize pharmacy IDs in claims and network data
    df = standardize_pharmacy_ids(df)
    network = standardize_network_ids(network)
    logger.info("Standardized pharmacy IDs in claims and network data")

    # Use centralized vectorized exclusion resolver
...
def create_data_filters(df):
    ...
    uni_neg = df[df["Open MDF Tier"].isin([2, 3]) & df["FormularyTier"].isin(["G", "GENERIC"])]
...
def create_network_data(df):
    ...
    logger.info(
        f"Sanity check: Excluded + Review + Non-excluded = {excluded_count + review_count + non_excluded_count} (should match total)"
    )
...
def write_excel_report(report_data, output_filename):
    ...
    write_audit_log("openmdf_bg2.py", f"Excel report written to: {output_path}", "INFO")
...
def process_data():
    ...
    write_audit_log("openmdf_bg2.py", f"Processing started by user: {username}", "INFO")
    ...
    try:
        ...
        if "reprice" not in file_paths or not file_paths["reprice"]:
            make_audit_entry("openmdf_bg2.py", "No reprice/template file provided.", "FILE_ERROR")
            write_audit_log("openmdf_bg2.py", "No reprice/template file provided.", status="ERROR")
        ...
        write_audit_log("openmdf_bg2.py", f"Processing complete for user: {username}", "INFO")
