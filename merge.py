    ...
    try:
        ...
        for col in REQUIRED_COLUMNS:
            if col not in df_merged.columns:
                ...
                write_shared_log("merge.py", f"Missing expected column: {col}", status="WARNING")
        ...
        try:
            ...
            if ws is not None and ws.max_row >= 1:
                ...
                if 'DATEFILLED' in header:
                    ...
                    write_shared_log("merge.py", "Applied date formatting successfully.")
                else:
                    logger.warning("DATEFILLED column not found for formatting.")
                    write_shared_log("merge.py", "DATEFILLED column not found for formatting.", status="WARNING")
            else:
                logger.warning("Worksheet is empty or not loaded, cannot apply formatting.")
                write_shared_log("merge.py", "Worksheet is empty or not loaded, cannot apply formatting.", status="WARNING")

        except Exception as ex:
            logger.warning(f"Failed to apply formatting: {ex}")
            write_shared_log("merge.py", f"Failed to apply formatting: {ex}", status="WARNING")
