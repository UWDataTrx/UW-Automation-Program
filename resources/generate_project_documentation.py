...
def generate_project_documentation_pdf():
    ...
    try:
        ...
        story.append(Paragraph("The UW Automation Program is a comprehensive Python-based application designed for healthcare repricing automation. It provides tools for data processing, file merging, template operations, and audit logging.", styles['Normal']))
        story.append(Spacer(1, 10))
        ...
        for category_name, category_info in categories.items():
            ...
            for file_path in category_files[:15]:  # Limit to first 15 files per category
                add_file_to_story(story, file_path, project_root, file_header_style, code_style, styles)
            
            if len(category_files) > 15:
                story.append(Paragraph(f"... and {len(category_files) - 15} more files in this category", styles['Italic']))
            
            story.append(PageBreak())
        
        # 9. Complete File Structure
        story.append(Paragraph("9. Complete File Structure", section_style))
        story.append(Paragraph("Complete directory tree of the project:", styles['Normal']))
        story.append(Spacer(1, 10))
...
def add_file_to_story(story, file_path, project_root, file_header_style, code_style, styles):
    ...
    try:
        ...
        if file_path.suffix == '.py':
            ...
            story.append(Paragraph(f"<b>Type:</b> {file_path.suffix.upper()} File", styles['Normal']))
            story.append(Paragraph("Binary or special file - content not displayed", styles['Italic']))
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            if item.is_dir() and current_depth < max_depth - 1:
                ...
                tree += generate_directory_tree(item, prefix + extension, max_depth, current_depth + 1)
