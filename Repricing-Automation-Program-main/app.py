...
class App:
    ...
    def _build_ui(self):
        ...
        self.button_frame.grid(
            row=1, column=0, columnspan=3, sticky="ew", pady=10, padx=10
        )
        ...
        self.notes_frame.grid(
            row=2, column=0, columnspan=4, sticky="ew", pady=10, padx=10
        )
        notes = ctk.CTkLabel(
            self.notes_frame,
            text=(
        ...
        self.start_disruption_button = ctk.CTkButton(
            self.root,
            text="Start Disruption",
            command=self.start_disruption,
            font=font_select,
            height=40,
            width=200,
            fg_color=LIGHT_COLORS["mint"],
        ...
        self.disruption_type_label = ctk.CTkLabel(
            self.dis_frame, text="Disruption Type:", font=font_select
        )
        self.disruption_type_label.grid(row=0, column=0, pady=10, padx=10)
        self.disruption_type_combobox = ctk.CTkComboBox(
            self.dis_frame,
            values=["Tier", "B/G", "OpenMDF (Tier)", "OpenMDF (B/G)"],
            variable=self.selected_disruption_type,
            width=200,
            dropdown_font=font_select,
            fg_color=LIGHT_COLORS["grey_blue"],
            dropdown_fg_color=LIGHT_COLORS["grey_blue"],
            dropdown_hover_color=LIGHT_COLORS["mint"],
            font=font_select,
            button_color=LIGHT_COLORS["mint"],
            button_hover_color=LIGHT_COLORS["mint"],
            border_color=LIGHT_COLORS["mint"],
        )
        self.disruption_type_combobox.grid(row=0, column=1, padx=10, pady=10)
        self.disruption_type_combobox.bind(
            "<<ComboboxSelected>>", self.select_disruption_type
        )

        # Progress
        self.prog_frame = ctk.CTkFrame(self.root, fg_color=LIGHT_COLORS["grey_blue"])
        self.prog_frame.grid(
            row=6, column=0, columnspan=3, sticky="ew", pady=10, padx=10
        )
    ...
    def update_progress(self, value):
        ...
        self.progress_label.configure(
            text=f"Progress: {percent}% | Est. {estimated}s left"
        )
    ...
    def start_disruption(self):
        ...
        if selected_type in program_files:
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success", f"{selected_type} disruption completed successfully."
                )
    ...
    def paste_into_template(self, processed_file):
        try:
            ...
            backup_name = os.path.basename(self.template_file_path).replace(
                ".xlsx", "_backup.xlsx"
            )
    ...
    def filter_template_columns(self, df):
        ...
        try:
            ...
            logger.warning(
                "Could not locate 'Client Name' to 'Logic' columns. Using full DataFrame."
            )
            return df
    ...
    def _start_process_internal(self):
        ...
        result = messagebox.askyesno(
            "Confirmation", "Are you sure you want to start the repricing process?"
        )
        if not result:
        ...
        try:
            ...
            def match_conditions(row, candidate):
                return (
                    ...
                    and row["DAYSUPPLY"] == -candidate["DAYSUPPLY"]
                    and row["NDC"] == candidate["NDC"]
                    and row["MemberID"] == candidate["MemberID"]
                    and row["Drug Name"] == candidate["Drug Name"]
                    and row["Pharmacy Name"] == candidate["Pharmacy Name"]
                    and candidate["DATEFILLED"] <= row["DATEFILLED"]
                    and abs(
                        row["Total AWP (Historical)"]
                        + candidate["Total AWP (Historical)"]
                    )
                    < 0.01
                )
            ...
            for idx, row in df[(df["QUANTITY"] < 0) & (df["Logic"] == "")].iterrows():
                ...
                matched = candidates[
                    candidates.apply(lambda c: match_conditions(row, c), axis=1)
                ]

                if not matched.empty:
                    closest = matched.loc[
                        (row["DATEFILLED"] - matched["DATEFILLED"]).abs().idxmin()
                    ]
                    df.at[idx, "Logic"] = "OR"
                ...
                if (
                    total_reversals > 0
                    and processed_reversals % max(1, total_reversals // 10) == 0
                ):
                    # Progress between 0.65 and 0.75
                    self.update_progress(
                        0.65 + 0.1 * processed_reversals / total_reversals
                    )
            ...
            excel_rows_to_highlight = [
                row_mapping[rid] for rid in unmatched_reversals if rid in row_mapping
            ]
            ...
            # Save as CSV
            df_sorted.drop_duplicates().to_csv(
                f"{opportunity_name} Claim Detail.csv", index=False
            )
    ...
    def highlight_unmatched_reversals(self, file_path):
        ...
        logging.info(
            f"Applied highlighting to {len(unmatched_rows)} rows in {file_path}"
        )
    ...
    def apply_theme_colors(self, colors):
        ...
        if hasattr(self, "start_disruption_button"):
            self.start_disruption_button.configure(
                fg_color=colors["mint"], text_color="#000000"
            )
